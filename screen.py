...
def _require_gdal() -> None:
    if not GDAL_AVAILABLE:
        raise RuntimeError(
            "GDAL/osgeo не установлен. "
            "Для make_screens нужен system GDAL (apt install gdal-bin libgdal-dev python3-gdal) "
            "или conda. В CI можно пропускать gdal-тесты."
        )
